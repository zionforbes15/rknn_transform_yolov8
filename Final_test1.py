...
def main():
    ...
    try:
        while True:
            ...
            # 提交处理请求并获取结果
            result_img, detected_distance = myFunc(pool.rknnPool[0], bgr_frame,
            ...
            if (time.monotonic() - uart.last_send_time) >= 0.02:
                if uart.send_command(current_cmd):
                    debug_str = f"[{time.strftime('%H:%M:%S')}] 指令: {current_cmd} "
                    if detected_distance is not None:
            ...
            cv2.putText(result_img, f"CMD: {current_cmd} | FPS: {fps:.1f}", 
